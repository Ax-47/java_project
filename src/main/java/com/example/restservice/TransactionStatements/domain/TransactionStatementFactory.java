    ...
    public static TransactionStatement create(
        ...
        return switch (type) {
            case PURCHASE -> new PurchaseStatement(
                    id, userId,
                    orderId.orElseThrow(
                            () -> new TransactionValidationException("orderId", "Order ID is required for PURCHASE")),
                    amount, method, status, referenceId, now);

            case TOPUP -> {
                if (orderId.isPresent()) {
                    throw new TransactionValidationException("orderId",
                            "Order ID must be null for top-up transactions");
                }
