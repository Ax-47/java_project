...
public class PurchaseStatement extends TransactionStatement {
  ...
  public PurchaseStatement(
      UUID id, UUID userId, UUID orderId, Credit amount,
      TransactionStatementsMethod method, TransactionStatementsStatus status,
      String referenceId, LocalDateTime createdAt) {

    super(id, userId, amount, method, status, referenceId, createdAt);
    this.orderId = Objects.requireNonNull(orderId, "Order ID is required for purchase transactions");
  }
